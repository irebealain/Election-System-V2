  ...
  try {
    ...
    if (ENVIRONMENT === "production") {
      if (!email.endsWith('@asyv.org')) {
        return res.status(403).json({ success: false, message: "Email must be from the organization domain" });
      }
    }
...
export const loginSuperAdmin = async (req, res) => {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      const superAdmin = await SuperAdmin.findOne({ email: mockEmail,
      googleId: mockGoogleId
      });
      if (!superAdmin) {
        return res.status(401).json({ success: false, message: "SuperAdmin not found, please sign up" });
      }
      res.status(200).json({
        success: true,
...
export const approveAdmin = async (req, res) => {
  ...
  try {
    ...
    const updatedAdmin = await Admin.findByIdAndUpdate(adminId, { status: 'approved' }, { new: true });
